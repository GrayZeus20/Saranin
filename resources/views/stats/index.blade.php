        ...
        <section class="bg-slate-900/50 border border-white/5 rounded-2xl p-5">
            ...
            <div class="space-y-1">
                ...
                <a href="{{ route('movie.show', $movie['id']) }}" class="flex items-center gap-3 py-2 px-2 rounded-xl hover:bg-white/5 transition-all group">
                    <span class="text-sm font-bold text-slate-500 w-6 text-right tabular-nums">{{ $i + 1 }}</span>
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-xs text-slate-500">{{ number_format($movie['popularity'] ?? 0) }} popularity</p>
                    </div>
                </a>
                @endforeach
            </div>
