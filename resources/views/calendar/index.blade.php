...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-8">
        ...
        <p class="text-slate-500 text-sm mt-1">What's playing and what's coming</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <div class="space-y-2">
                ...
                <a href="{{ route('movie.show', $movie['id']) }}" class="flex gap-3.5 p-3 rounded-xl hover:bg-white/5 transition-all group">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-xs text-slate-600 line-clamp-2 mt-1">{{ $movie['overview'] }}</p>
                        @endif
                    </div>
                </a>
                @empty
                <p class="text-slate-500 text-sm py-4">No movies currently playing.</p>
                @endforelse
            </div>
        </section>
        ...
        <section class="bg-slate-900/50 border border-white/5 rounded-2xl p-5">
            ...
            <div class="space-y-2">
                ...
                @empty
                <p class="text-slate-500 text-sm py-4">No upcoming movies found.</p>
                @endforelse
            </div>
        </section>
    </div>
